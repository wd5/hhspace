...
class Video(models.Model):
    ...
    def convert(self):
        ...
        tmp_video = '' + self.video.name[:self.video.name.rfind('.')] + '_tmp.mp4'
        qtvideo = '' + self.video.name[:self.video.name.rfind('.')] + '.mp4'
        self.photo = 'screenshots/' + self.video.name[:self.video.name.rfind('.')] + '.jpg'
        logging.info("Video is : %s" % self)


        os.system('%s -y -i "%s" -an -ss 21 -r 1 -vframes 1 -y -f mjpeg -s 150x130 "%s"' % (settings.patch.get('FFMPEG' or 'ffmpeg'), self.video.name, self.photo))
        logging.info('FFMPEG CONVERT: %s -i %s -threads 8  -vcodec libx264 -ar 22050 -ab 56k -vpre slow -aspect 4:3 -b 350k -r 12 -f mp4 -s 350x250 -acodec libfaac -ac 2 %s' % (settings.patch.get('FFMPEG' or 'ffmpeg'), self.video.name, tmp_video))
        os.system('%s -i %s -vcodec libx264 -ar 22050 -ab 56k -vpre slow -aspect 4:3 -b 350k -r 12 -f mp4 -s 480x230 -acodec libfaac -ac 2 %s' % (settings.patch.get('FFMPEG' or 'ffmpeg'), self.video.name, tmp_video))
